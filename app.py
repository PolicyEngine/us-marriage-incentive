...
def get_combined_data(programs, index, tab_name):
    ...
    return table.create_table_data(
        categories, married_values, separate_values, tab_name
    )
...
def display_dataframe(df, tab_name):
    ...
    fig = heatmap.create_heatmap_chart(
        state_code, children_ages, tab_name, disability_status
    )
...
if submit:
    ...
    total_data = table.create_table_data(
    ...
    with tab3:
        display_dataframe(
            all_data[all_data["Tab"] == "Refundable Credits"], "Refundable Tax Credits"
        )
    with tab4:
        display_dataframe(
            all_data[all_data["Tab"] == "Taxes before Refundable Credits"],
...
st.markdown("***")
st.markdown(
    "*We attribute all dependents to the head of household when considering unamrried filers*"
)
